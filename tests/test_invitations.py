...
async def test_our_clock_matches_the_one_created_at_is_written_on(tx, household):
    ...
    before = _now()
    person, _ = await allowlist(me, "clock@example.test")
    after = _now()

    assert before - timedelta(seconds=5) <= person.created_at
    assert person.created_at <= after + timedelta(seconds=5)
...
async def test_invite_to_reef_allowlists_without_any_membership(tx, household):
    ...
    person = (
        await Person.objects().where(Person.email == "curious@example.test").first()
    )
    assert person is not None
    assert person.display_name == "Curious"
    assert person.invited_by_person_id == household["wouter"].id
    # The whole point: allowlisted, but shown nothing.
    assert await Membership.count().where(Membership.person_id == person.id) == 0
...
async def test_budget_boundary_last_one_lands_next_one_refused(tx, household):
    ...
    assert (
        ...
        .where(Person.email == "one-too-many@example.test")
        .first()
    ) is None


async def test_entries_older_than_the_window_do_not_count(tx, household):
    me = principal_for(household["wouter"])
    await spend(me, INVITE_BUDGET)
    ...
    await Person.update({Person.created_at: stale}).where(
        Person.invited_by_person_id == me.person_id
    )
...
async def test_an_address_reef_already_knows_is_free(tx, household):
    ...
    person, created = await allowlist(me, household["partner"].email)
    assert created is False
    assert person.id == household["partner"].id
...
async def test_email_is_normalised_before_it_is_stored(tx, household):
    me = principal_for(household["wouter"])
    person, created = await allowlist(me, "  MiXeD@Example.TEST  ")
    assert person.email == "mixed@example.test"
    assert created is True
