...
policy system possible: that ``reef_cove_ids()`` reads ``memberships``
...
from reef.models import Cove, Membership
...
async def test_the_helper_functions_are_owned_by_the_bypassing_role():
    ...
    rows = await DB._run_in_new_connection(
        ...
        "WHERE p.proname IN ('reef_cove_ids', 'reef_member_cove_ids')"
    )
...
async def test_the_functions_are_security_definer_with_a_fixed_search_path():
    ...
    rows = await DB._run_in_new_connection(
        ...
        "WHERE proname IN ('reef_cove_ids', 'reef_member_cove_ids')"
    )
    for row in rows:
...
async def test_the_functions_are_closed_to_public(tx, household):
    ...
    granted = await DB._run_in_new_connection(
        "SELECT has_function_privilege('public', 'reef_cove_ids()', 'EXECUTE') AS pub"
    )
    assert granted[0]["pub"] is False


async def test_cove_ids_returns_only_the_armed_principals_coves(tx, household):
    """The core contract: one principal, their memberships, nobody else's."""
    await _arm(household["wouter"])
    rows = await DB._run_in_new_connection("SELECT reef_cove_ids() AS id")
    ...
    async with DB.transaction():
        await _arm(household["wouter"])
        mine = {r["id"] for r in await Cove.raw("SELECT reef_cove_ids() AS id")}
        assert mine == {household["w_personal"].id, household["shared"].id}
        assert household["p_personal"].id not in mine


async def test_member_cove_ids_excludes_viewer_memberships(tx, household, graph):
    ...
    readable = {r["id"] for r in await Cove.raw("SELECT reef_cove_ids() AS id")}
    writable = {r["id"] for r in await Cove.raw("SELECT reef_member_cove_ids() AS id")}
...
async def test_an_unarmed_transaction_reaches_no_coves(tx, household):
    """Fail closed: no principal means no rows, not all rows."""
    assert await Cove.raw("SELECT reef_cove_ids() AS id") == []


async def test_a_cleared_principal_reaches_no_coves(tx, household):
    """The clearing path sets '' rather than unsetting; NULLIF must fold it."""
    await _arm(household["wouter"])
    await Cove.raw("SELECT set_config('app.person_id', '', true)")
    assert await Cove.raw("SELECT reef_cove_ids() AS id") == []
...
async def test_a_policy_calling_the_function_on_its_own_table_does_not_recurse(graph):
    ...
    personal = await graph.personal_cove(wouter, slug="recursion-personal")
    await graph.personal_cove(partner, slug="recursion-partner")
    shared = await graph.shared_cove("recursion-shared", wouter, partner)
    ...
    try:
        async with DB.transaction():
            await _arm(wouter)
            visible = {
                (row["person_id"], row["cove_id"])
                for row in await Membership.select(
                    Membership.person_id, Membership.cove_id
                )
            }
        # Wouter's own two memberships, plus partner's membership of the
        # shared cove -- a row in a cove Wouter reaches. Partner's personal
        # membership is not.
        assert visible == {
