...
Some operations legitimately reach past the policies: admitting somebody to a
cove, removing them, erasing an account, handing a cove to a successor. They
run inside ``SECURITY DEFINER`` functions owned by a ``BYPASSRLS`` role
because no row policy can express them without permitting far more. That is
the right design, and it means the guarantee those four operations carry is
*accountability*, not *prevention*.

So each one is recorded. The honest claim this supports is narrow and worth
stating exactly: administrative acts leave a trail outside the database, on a
service the application cannot rewrite, promptly enough that erasing them
afterwards requires a second, different kind of access. It does **not** make
reef secure from whoever can deploy it -- nothing here does, and the module
that ships telemetry says the same thing at more length.
...
from rif.telemetry import is_configured

# The complete set. Adding one is a deliberate act: it means a new way for
# authority to be exercised outside the policies, which is worth noticing in
# review rather than discovering in a log.
