...
_FROZEN_ENABLE = [
    "CREATE OR REPLACE FUNCTION reef_cove_ids() RETURNS SETOF uuid LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_catalog AS $reef$SELECT cove_id FROM memberships WHERE person_id = NULLIF(current_setting('app.person_id', true), '')::uuid$reef$",
    ...
    "CREATE POLICY pages_insert ON pages FOR INSERT WITH CHECK (cove_id IN (SELECT reef_member_cove_ids()))",
    "CREATE POLICY pages_update ON pages FOR UPDATE USING (cove_id IN (SELECT reef_member_cove_ids())) WITH CHECK (cove_id IN (SELECT reef_member_cove_ids()))",
    "CREATE POLICY pages_delete ON pages FOR DELETE USING (cove_id IN (SELECT reef_member_cove_ids()))",
    "ALTER TABLE attachments ENABLE ROW LEVEL SECURITY",
    "ALTER TABLE attachments FORCE ROW LEVEL SECURITY",
    "CREATE POLICY attachments_select ON attachments FOR SELECT USING (cove_id IN (SELECT reef_cove_ids()))",
    "CREATE POLICY attachments_insert ON attachments FOR INSERT WITH CHECK (cove_id IN (SELECT reef_member_cove_ids()))",
    "CREATE POLICY attachments_update ON attachments FOR UPDATE USING (cove_id IN (SELECT reef_member_cove_ids())) WITH CHECK (cove_id IN (SELECT reef_member_cove_ids()))",
    "CREATE POLICY attachments_delete ON attachments FOR DELETE USING (cove_id IN (SELECT reef_member_cove_ids()))",
    "ALTER TABLE revisions ENABLE ROW LEVEL SECURITY",
    "ALTER TABLE revisions FORCE ROW LEVEL SECURITY",
    "CREATE POLICY revisions_select ON revisions FOR SELECT USING (page_id IN (SELECT id FROM pages WHERE cove_id IN (SELECT reef_cove_ids())))",
    "CREATE POLICY revisions_insert ON revisions FOR INSERT WITH CHECK (page_id IN (SELECT id FROM pages WHERE cove_id IN (SELECT reef_member_cove_ids())))",
    "CREATE POLICY revisions_update ON revisions FOR UPDATE USING (page_id IN (SELECT id FROM pages WHERE cove_id IN (SELECT reef_member_cove_ids()))) WITH CHECK (page_id IN (SELECT id FROM pages WHERE cove_id IN (SELECT reef_member_cove_ids())))",
    "CREATE POLICY revisions_delete ON revisions FOR DELETE USING (page_id IN (SELECT id FROM pages WHERE cove_id IN (SELECT reef_member_cove_ids())))",
