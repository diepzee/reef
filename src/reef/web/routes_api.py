...
from reef.models import Page, Person
...
from reef.web.routes_auth import WORKOS_LOGOUT_URL
...
# mirrors the pattern in reef.web.routes_auth.
...
def api(handler: Callable) -> Callable:
    ...
    async def endpoint(request: Request) -> Response:
        ...
        if response.headers.get(_ACCOUNT_DELETED_HEADER) == "1":
            ...
            # every day. See reef.web.session.
            set_session_cookie(
                response,
                principal,
...
def _feed_path() -> Path:
    ...
    same way :mod:`reef.web.static` wraps it.
