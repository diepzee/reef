...
async def household(session) -> dict:
    ...
    await session.flush()
    w_personal = Space(
        slug="wouter", kind=SpaceKind.PERSONAL, owner_person_id=wouter.id
    )
    p_personal = Space(
        slug="partner", kind=SpaceKind.PERSONAL, owner_person_id=partner.id
    )
...
async def test_content_is_invisible_without_a_principal(session, household):
    ...
    session.add(
        Page(space_id=household["w_personal"].id, path="a.md", title="a", body="secret")
    )
    await session.flush()
...
async def test_raw_select_as_other_principal_returns_nothing(session, household):
    ...
    assert (
        await session.scalars(select(Page).where(Page.path == "health.md"))
    ).all() == []
...
async def test_each_write_snapshots_full_state_and_bumps_versions(session, household):
    me = principal_for(household["wouter"])
    await save_page(
        session, me, "personal", "a.md", "one", message="first", title="A", tags=["t"]
    )
    await save_page(session, me, "personal", "a.md", "two", message="second")
    revisions = (
        await session.scalars(select(Revision).order_by(Revision.created_at))
    ).all()
...
async def test_stale_expected_version_conflicts(session, household):
    ...
    with pytest.raises(VersionConflict):
        await save_page(
            session, me, "personal", "a.md", "clobber", message="x", expected_version=0
        )
...
async def test_meta_paths_are_protected_from_generic_writes(session, household):
    ...
    with pytest.raises(ProtectedPath):
        await save_page(
            session, me, "personal", "meta/persona.md", "hijacked", message="x"
        )
    ...
    return list(
        (
            await session.scalars(
                select(Page).where(Page.space_id == space.id).order_by(Page.path)
            )
        ).all()
    )
...
async def test_context_never_includes_the_other_persons_space(session, household):
    ...
    bodies = [
        p["body"]
        for s in (await load_context(session, mine, char_budget=100_000)).spaces
        for p in s.pages
    ]
...
async def load_context(
    ...
    pages = list(
        (await session.scalars(select(Page).where(Page.space_id.in_(space_ids)))).all()
    )
    attachments = list(
        (
            await session.scalars(
                select(Attachment).where(
                    Attachment.space_id.in_(space_ids),
    ...
    version = ";".join(f"{space.slug}={space.version}" for space in spaces) or "empty"
    return ContextPayload(
        version=f"{principal.person_id}:{version}",
        truncated=truncated,
        note=note,
...
async def principal_from_claims(session: AsyncSession, claims: dict) -> Principal:
    ...
    if person is None:
        ...
        person = await session.scalar(
            select(Person).where(
                Person.email == email.lower(), Person.subject.is_(None)
            )
        )
        if person is None:
...
async def list_spaces() -> list[dict]:
    ...
    async with session_scope() as session:
        principal = await current_principal(session)
        return [
            {"alias": s.kind.value, "slug": s.slug, "version": s.version}
...
async def test_tool_read_page_not_found_is_plain(session, household):
    me = principal_for(household["wouter"])
    assert (await tool_read_page(session, me, "personal", "nope.md"))[
        "error"
    ] == "not_found"
```
...
async def test_remember_can_target_household_explicitly(session, household):
    me = principal_for(household["wouter"])
    await tool_remember(
        session, me, "school run swaps to me on Fridays", space="household"
    )
...
async def update_meta_page(
    ...
    async with session_scope() as session:
        principal = await current_principal(session)
        page = await save_page(
            session, principal, space, path, body, message=message, allow_protected=True
        )
        return {"space": space, "path": page.path, "version": page.version}
```
...
async def test_full_flow_copies_stubs_and_discloses(session, household):
    ...
    assert (
        await get_page(session, theirs, "household", "boiler.md")
    ).body == "Vaillant ecoTEC"
...
async def test_confirm_retry_is_idempotent_and_never_copies_the_stub(
    session, household
):
    me = principal_for(household["wouter"])
    theirs = principal_for(household["partner"])
    ...
    assert (
        await get_page(session, theirs, "household", "b.md")
...
async def test_personal_attachment_is_invisible_to_the_other_person(session, household):
    ...
    assert (
        await get_attachment(session, theirs, "personal", attachment.object_key) is None
    )
...
async def test_unknown_alias_is_denied(session, household):
    ...
    with pytest.raises(AccessDenied):
        await add_attachment(
            session, me, "theirs", b"x", "image/png", description="x", store=FakeStore()
        )
```
...
async def add_attachment(
    ...
    *,
    ...
    if page_path is not None:
        page = await session.scalar(
            select(Page).where(Page.space_id == space.id, Page.path == page_path)
        )
...
async def _main(alias: str, target: str) -> None:
    ...
    async with session_scope() as session:
        person = await session.scalar(
            select(Person).where(Person.email == os.environ["RIF_DEV_PRINCIPAL_EMAIL"])
        )
        principal = Principal(person_id=person.id, email=person.email)
