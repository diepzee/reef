...
Does the closest product to reef already exist? reef's specific combination:
...
What it lacks vs reef: per-seat B2B/prosumer pricing that does not map to a
...
What it lacks vs reef: memory is item-based snippets, not a wiki with
...
than reef's per-space RLS; attribution, validity windows, as-of-date
...
What it lacks vs reef: a single self-hosted Rust binary with no hosted
...
- **Memory-MCP by Cedra** (Cloudflare Workers + D1) — remote team memory
  ...
  no longer unique to reef. Static per-teammate bearer tokens, no private
...
- **The gap is real but likely not durable.** Memlord already gestures at
  ...
  exists. The consumer/household position is open today and reef is,
...
glama.ai is the largest MCP directory (~72,000 servers). reef is listed
there as `diepzee/reef`, categorised under Knowledge & Memory, RAG
Systems, and Note Taking. This sweep asked the reverse of the question
above: not "what does reef have that nobody else does" but **"what does
the field have that reef does not"** — and then, per capability, whether
reef should close the gap, adapt the idea, or refuse it on purpose.

## Who is on glama that resembles reef

**Multi-user shared memory (reef's own category).** Basic Memory
...
Traction check: among the reef-like group, only Basic Memory has real
...
is reef's feature set re-imagined for engineering teams and coding
...
2. **Automatic capture** — Mem0 auto-extracts and compresses; Amber
   ...
   Copilot transcripts; Lore auto-stores via hooks. reef writes only
   deliberately.
3. **Hook-time injection** — Pathrule injects path-relevant team
   ...
   ~20 ms; docmancer bakes memory into always-loaded context. reef
   depends on the model choosing to call `load_index`.
4. **Knowledge-graph structure** — entities, relations, traversal
   (official server, Graphiti, Cognee, Lore, SharedMemory). reef stores
   prose pages.
5. **Temporal reasoning** — Lians answers point-in-time recall with
   ...
   with supersession chains; Graphiti's whole model is temporal. reef
...
6. **Memory lifecycle** — decay, TTL, consolidation, contradiction
   ...
   compression). reef pages live until someone edits them.
7. **Self-hosting / local-first** — most of the field runs on your own
   machine; Pathrule and Basic Memory offer it as a tier. reef is
...
8. **Org machinery and developer APIs** — RBAC roles, SSO, audit logs,
   ...
   (Mem0, Zep, Letta). reef has none of these.

## How reef beats them, gap by gap
...
returns someone else's memories. In reef, a search that forgets a filter
...
records is the creepy thing reef exists not to be. But the *labour
problem* is real. reef's answer is already half-built: `remember`
...
memory must arrive without being asked for. reef's equivalent is
...
delete quietly; reef's asks.

**Refuse — knowledge graphs.** Triples are for machines; reef's readers
...
abandoning prose. Anyone who wants entity extraction is not reef's
customer.
...
and reef's trust story is already load-bearing without it: RLS enforced
...
APIs serve markets reef is deliberately not in. Basic Memory and Mem0
can keep them. The moment reef grows roles and seat pricing it becomes
